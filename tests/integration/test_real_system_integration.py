...
async def real_message_broker(real_redis) -> AsyncGenerator[MessageBroker, None]:
    ...
    settings = get_settings()
    broker = MessageBroker(redis_url=settings.redis_url)
...
class TestRealSystemIntegration:
    ...
    @pytest.mark.asyncio
    async def test_real_system_performance_baseline(self, real_task_queue: TaskQueue):
        """Test system performance with real components."""
        import time

        # Performance test: submit/get operations
        num_tasks = 50  # Reduced for CI stability
        ...
        for _ in range(num_tasks):
            task = await real_task_queue.get_task()
            if task is None:
                break
            retrieved_count += 1
        ...
        assert submit_time < 10.0  # Should submit 50 tasks in under 10 seconds
        assert retrieve_time < 10.0  # Should retrieve 50 tasks in under 10 seconds
        assert retrieved_count == num_tasks  # All tasks should be processed

        # Test queue statistics
        total_tasks = await real_task_queue.get_total_tasks()
        assert total_tasks >= num_tasks  # Should have processed at least our test tasks
