...
class CLIToolManager:
    ...
    def _detect_available_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        # Check Gemini CLI
        if self._check_tool_available("gemini", ["--version"]) or \
           self._check_tool_available("gcloud", ["ai", "--version"]):
    ...
    def _select_preferred_tool(self) -> Optional[CLIToolType]:
        ...
        if settings.preferred_cli_tool and settings.preferred_cli_tool in self.available_tools:
    ...
    async def execute_prompt(self, prompt: str, tool_override: Optional[CLIToolType] = None) -> ToolResult:
        ...
        try:
            ...
            if result.success:
                logger.info("CLI tool execution successful", 
                           tool=tool_to_use.value,
                           execution_time=result.execution_time)
                return result
            else:
                logger.warning("CLI tool execution failed, trying fallback",
                             tool=tool_to_use.value,
                             error=result.error)
        
        except Exception as e:
            logger.error("CLI tool execution error", 
                        tool=tool_to_use.value,
                        error=str(e))
        ...
        for fallback_tool in self.available_tools:
            ...
            try:
                ...
                if result.success:
                    logger.info("Fallback CLI tool execution successful",
                               tool=fallback_tool.value,
                               execution_time=result.execution_time)
                    return result
                    
            except Exception as e:
                logger.error("Fallback CLI tool execution error",
                            tool=fallback_tool.value,
                            error=str(e))
...
class BaseAgent(ABC):
    ...
    def _setup_message_handlers(self):
        ...
        self.message_handler.register_handler("task_assignment", self._handle_task_assignment)
    ...
    async def execute_with_cli_tool(self, 
                                   prompt: str, 
                                   tool_override: Optional[CLIToolType] = None) -> ToolResult:
    ...
    async def _process_task_implementation(self, task: Task) -> TaskResult:
        ...
        context_text = "\n".join([
            f"Context: {r.context.content[:200]}..."
    ...
    async def send_message(self, 
                          ...
                          message_type: MessageType = MessageType.DIRECT) -> str:
    ...
    async def health_check(self) -> HealthStatus:
        ...
        try:
            ...
            if self.tasks_failed > self.tasks_completed * 0.5:  # More than 50% failure rate
                return HealthStatus.DEGRADED
    ...
    async def _register_agent(self):
        ...
        try:
            ...
            existing_agent = db_session.query(AgentModel).filter_by(name=self.name).first()
            
            if existing_agent:
                # Update existing agent
                existing_agent.status = "active"
                existing_agent.last_heartbeat = time.time()
            else:
                # Create new agent
                agent = AgentModel(
                    name=self.name,
    ...
    async def _record_task_metrics(self, task: Task, result: TaskResult, execution_time: float):
    ...
    async def _handle_health_check(self, message: Message) -> Dict[str, Any]:
        ...
        return {
            ...
            "preferred_cli_tool": self.cli_tools.preferred_tool.value if self.cli_tools.preferred_tool else None
        }
