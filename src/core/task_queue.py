...
class TaskQueue:
    ...
    async def submit_task(self, task: Task) -> str:
        ...
        if await self._dependencies_satisfied(task):
            ...
            logger.info("Task submitted to queue", task_id=task.id, priority=task.priority)
        else:
            ...
            logger.info("Task waiting for dependencies", task_id=task.id, dependencies=task.dependencies)
        
        # Update stats
        await self._update_stats("submitted")
        return task.id
    
    async def get_task(self, agent_id: str, priorities: Optional[List[int]] = None, timeout: int = 60) -> Optional[Task]:
        ...
        for priority in sorted(priorities):
            ...
            logger.info("Task assigned", task_id=task_id, agent_id=agent_id, priority=priority)
    ...
    async def fail_task(self, task_id: str, error: str, retry: bool = True) -> bool:
        ...
        if retry and task.retry_count < task.max_retries:
            ...
            logger.info("Task scheduled for retry", task_id=task_id, retry_count=task.retry_count, delay=delay)
            return True
        else:
            ...
            if await self.redis_client.sismember(dep_key, completed_task_id):
                ...
                if not remaining_deps:
                    ...
                    if task_data:
                        ...
                        logger.info("Task dependencies satisfied, queued", task_id=task_id)
