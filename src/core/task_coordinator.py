...
class TaskCoordinator:
        ...
        try:
            ...
            try:
                ...
                for agent in agents:
                    ...
                    if agent_type == "meta":
                        self._add_agent_capability(agent_name, "system_analysis", 0.9, 0.85, 45.0)
                        self._add_agent_capability(agent_name, "coordination", 0.95, 0.9, 30.0)
                        self._add_agent_capability(agent_name, "planning", 0.8, 0.8, 60.0)
                    
                    elif agent_type == "developer":
                        self._add_agent_capability(agent_name, "development", 0.85, 0.8, 120.0)
                        self._add_agent_capability(agent_name, "coding", 0.9, 0.85, 90.0)
                        self._add_agent_capability(agent_name, "implementation", 0.8, 0.75, 150.0)
                        self._add_agent_capability(agent_name, "refactoring", 0.7, 0.7, 180.0)
                    
                    elif agent_type == "qa":
                        self._add_agent_capability(agent_name, "testing", 0.9, 0.85, 60.0)
                        self._add_agent_capability(agent_name, "validation", 0.85, 0.8, 45.0)
                        self._add_agent_capability(agent_name, "qa", 0.95, 0.9, 75.0)
                        self._add_agent_capability(agent_name, "debugging", 0.8, 0.75, 90.0)
                    
                    elif agent_type == "architect":
                        self._add_agent_capability(agent_name, "architecture", 0.9, 0.85, 180.0)
                        self._add_agent_capability(agent_name, "design", 0.85, 0.8, 120.0)
                        self._add_agent_capability(agent_name, "planning", 0.8, 0.75, 90.0)
                        self._add_agent_capability(agent_name, "analysis", 0.75, 0.7, 60.0)
                    
                    elif agent_type == "research":
                        self._add_agent_capability(agent_name, "research", 0.9, 0.85, 300.0)
                        self._add_agent_capability(agent_name, "analysis", 0.8, 0.75, 180.0)
                        self._add_agent_capability(agent_name, "learning", 0.85, 0.8, 240.0)
    ...
    def _add_agent_capability(self, agent_name: str, task_type: str, 
                            proficiency: float, success_rate: float, 
                            avg_time: float) -> None:
    ...
    def _select_assignment_strategy(self, task: Task) -> AssignmentStrategy:
        ...
        if task.type in ["development", "testing", "architecture", "research"]:
            return AssignmentStrategy.CAPABILITY_BASED
        
        # System tasks use specialized assignment
        if task.type in ["system_analysis", "coordination", "monitoring"]:
            return AssignmentStrategy.SPECIALIZED
        
        # Default to load-balanced assignment
        return AssignmentStrategy.LOAD_BALANCED
    
    async def _find_best_agent(self, task: Task, strategy: AssignmentStrategy) -> Optional[str]:
    ...
    def _get_available_agents(self) -> List[str]:
        ...
        for agent_name, workload in self.agent_workloads.items():
            ...
            if workload.current_tasks < workload.max_capacity:
                ...
                if agent_name in self.agent_capabilities:
                    if any(cap.availability for cap in self.agent_capabilities[agent_name].values()):
                        available.append(agent_name)
        
        return available
    
    def _find_best_agent_by_capability(self, task: Task, agents: List[str]) -> Optional[str]:
    ...
    def _find_best_agent_by_performance(self, task: Task, agents: List[str]) -> Optional[str]:
    ...
    def _find_specialized_agent(self, task: Task, agents: List[str]) -> Optional[str]:
        ...
        preferred_types = specializations.get(task.type, [])
        ...
        for agent_name in agents:
            try:
                ...
                try:
                    agent = db_session.query(AgentModel).filter_by(name=agent_name).first()
    ...
    def _calculate_assignment_confidence(self, task: Task, agent_name: str, 
    ...
    def _estimate_completion_time(self, task: Task, agent_name: str) -> float:
        ...
        if (agent_name in self.agent_capabilities and 
            task.type in self.agent_capabilities[agent_name]):
            capability = self.agent_capabilities[agent_name][task.type]
        ...
        for assignment in self.task_assignments.values():
            ...
            if time_since_assignment > stuck_threshold:
                ...
                if task and task.status == TaskStatus.RUNNING:
                    logger.warning("Potentially stuck task detected",
    ...
    async def _update_capability_assessments(self) -> None:
        ...
        for task in recent_tasks:
            if task.assigned_to and task.assigned_to in self.agent_capabilities:
                ...
                execution_time = (task.completed_at or time.time()) - (task.started_at or task.created_at)
    ...
    async def get_coordination_status(self) -> Dict[str, Any]:
        ...
        if self.task_assignments:
            avg_confidence = sum(a.confidence_score for a in self.task_assignments.values()) / len(self.task_assignments)
        else:
            avg_confidence = 0.0
