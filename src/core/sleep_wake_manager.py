...
from .advanced_context_engine import get_advanced_context_engine, MemoryConsolidationStats
...
class PerformanceMonitor:
    ...
    async def collect_performance_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            metrics["task_success_rate"] = completed_tasks / max(1, completed_tasks + failed_tasks)
            ...
            try:
                ...
                if recent_metrics:
                    task_times = [m.value for m in recent_metrics if m.metric_name == "task_execution_time"]
                    if task_times:
    ...
    def suggest_optimizations(self) -> List[str]:
        ...
        if current_metrics.get("memory_usage", 0) > 75:
            optimizations.append("Implement memory optimization or increase system memory")
        
        if current_metrics.get("cpu_usage", 0) > 70:
            optimizations.append("Optimize CPU-intensive operations or scale horizontally")
        ...
        if queue_depth > active_agents * 5:
            optimizations.append("Agent to task ratio suboptimal - consider scaling agents")
...
class SleepWakeManager:
        ...
        while True:
            try:
                if self.current_state == SystemState.AWAKE:
                    ...
                    # Sleep cycle should be handled by _execute_sleep_cycle
                    pass
    ...
    async def _should_sleep_adaptive(self) -> bool:
        ...
        try:
            ...
            if len(performance_issues) >= 3:
                logger.info("Performance degradation detected, initiating early sleep for optimization")
                return True
            ...
            if memory_stats.get("total_contexts", 0) > 10000:
                logger.info("High memory usage detected, initiating sleep for consolidation")
                return True
    ...
    async def _monitor_awake_period(self) -> None:
        ...
        if len(self.workload_history) % 30 == 0:  # Every 30 minutes
            logger.info("System awake status",
        ...
        try:
            ...
            try:
                ...
                for agent in agents:
                    ...
                    logger.debug("Memory consolidated for agent",
                               agent=agent.name,
    ...
    async def _extract_patterns(self) -> int:
        ...
        try:
            ...
            logger.info("Pattern extraction completed", patterns_discovered=patterns_discovered)
        ...
        try:
            ...
            if self.performance_monitor.baseline_metrics:
                for metric, value in final_metrics.items():
                    baseline = self.performance_monitor.baseline_metrics.get(metric, value)
                    if baseline > 0:
    ...
    async def _wake_up(self) -> None:
        ...
        try:
            ...
            logger.info("System wake completed",
                       restoration_duration_ms=restoration_duration,
                       agents_restored=agents_restored,
                       system_health=health_score)
    ...
    def get_sleep_stats(self) -> Dict[str, Any]:
        ...
        avg_duration = sum(s.duration_hours for s in self.sleep_metrics_history) / total_cycles
        avg_patterns = sum(s.patterns_discovered for s in self.sleep_metrics_history) / total_cycles
