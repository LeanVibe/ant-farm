...
class Settings(BaseSettings):
    ...
    preferred_cli_tool: Optional[CLIToolType] = Field(default=None, env="PREFERRED_CLI_TOOL")
    ...
    database_url: str = Field(
        default="postgresql://hive_user:hive_pass@localhost:5432/leanvibe_hive",
        env="DATABASE_URL"
    )
    ...
    embedding_provider: EmbeddingProvider = Field(default=EmbeddingProvider.SENTENCE_TRANSFORMERS, env="EMBEDDING_PROVIDER")
    ...
    message_retention_hours: int = Field(default=168, env="MESSAGE_RETENTION_HOURS")  # 7 days
    ...
    context_compression_threshold: int = Field(default=5, env="CONTEXT_COMPRESSION_THRESHOLD")
    context_importance_decay_days: int = Field(default=30, env="CONTEXT_IMPORTANCE_DECAY_DAYS")
    ...
    improvement_proposal_threshold: float = Field(default=0.1, env="IMPROVEMENT_PROPOSAL_THRESHOLD")
    ...
    def validate_configuration(self) -> List[str]:
        ...
        if self.is_production:
            ...
            if not self.anthropic_api_key and not self.openai_api_key:
                issues.append("At least one API key (Anthropic or OpenAI) should be set in production")
