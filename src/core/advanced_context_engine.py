...
from .context_engine import EmbeddingService, EmbeddingProvider
...
class PatternRecognizer:
    ...
    async def discover_patterns(self, contexts: List[Context]) -> List[KnowledgePattern]:
    ...
    async def _extract_pattern(self, contexts: List[Context]) -> Optional[KnowledgePattern]:
    ...
    async def _discover_temporal_patterns(self, contexts: List[Context]) -> List[KnowledgePattern]:
    ...
    async def _discover_outcome_patterns(self, contexts: List[Context]) -> List[KnowledgePattern]:
        """Discover patterns related to task outcomes."""
        patterns = []
        
        success_contexts = [ctx for ctx in contexts if "success" in ctx.content.lower()]
        failure_contexts = [ctx for ctx in contexts if "error" in ctx.content.lower() or "fail" in ctx.content.lower()]
...
class MemoryConsolidator:
    ...
    async def consolidate_memory(self, db_session: Session, agent_id: str) -> MemoryConsolidationStats:
    ...
    async def _merge_related_contexts(self, db_session: Session, context: Context) -> None:
        """Merge context with related contexts."""
        # Find related contexts
        related = db_session.query(Context).filter(
            and_(
...
class AdvancedContextEngine:
    ...
    async def store_context(self, 
        ...
        context = Context(
            id=hashlib.md5(f"{agent_id}_{content}_{time.time()}".encode()).hexdigest()[:16],
            agent_id=agent_id,
            content=content,
            embedding=embedding,
        ...
        try:
            ...
            if len(self.working_memory_cache[agent_id]) > self.cache_max_size:
                self.working_memory_cache[agent_id] = self.working_memory_cache[agent_id][-self.cache_max_size:]
    ...
    def _determine_initial_layer(self, importance_score: float, context_type: ContextType) -> MemoryLayer:
    ...
    async def retrieve_context(self,
                             ...
                             include_patterns: bool = True) -> List[ContextSearchResult]:
        ...
        final_results = sorted(ranked_results, key=lambda r: r.relevance_score, reverse=True)[:limit]
    ...
    async def _search_cache(self, query_vector: np.ndarray, 
                          cache_contexts: List[Context], 
                          limit: int) -> List[ContextSearchResult]:
    ...
    async def _search_database(self, query_vector: np.ndarray,
        ...
        try:
            ...
            contexts = db_session.query(Context).filter(
                and_(*filters)
            ).order_by(desc(Context.importance_score)).limit(limit * 2).all()
    ...
    def _rank_search_results(self, results: List[ContextSearchResult], query: str) -> List[ContextSearchResult]:
    ...
    async def _search_patterns(self, query: str, agent_id: str) -> List[ContextSearchResult]:
    ...
    async def consolidate_memory(self, agent_id: str) -> MemoryConsolidationStats:
        ...
        try:
            ...
            stats = await self.memory_consolidator.consolidate_memory(db_session, agent_id)
            
            # Discover new patterns
            contexts = db_session.query(Context).filter(
                Context.agent_id == agent_id
            ).order_by(desc(Context.created_at)).limit(100).all()
    ...
    async def get_memory_stats(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            # Basic counts
            total_contexts = db_session.query(func.count(Context.id)).filter(
                Context.agent_id == agent_id
            ).scalar()
            ...
            for layer in MemoryLayer:
                count = db_session.query(func.count(Context.id)).filter(
                    and_(
                        Context.agent_id == agent_id,
            ...
            importance_stats = {
                "high": db_session.query(func.count(Context.id)).filter(
                    and_(Context.agent_id == agent_id, Context.importance_score >= 0.7)
                ).scalar(),
                "medium": db_session.query(func.count(Context.id)).filter(
                    and_(Context.agent_id == agent_id, 
                         Context.importance_score >= 0.3,
                         Context.importance_score < 0.7)
                ).scalar(),
                "low": db_session.query(func.count(Context.id)).filter(
...
async def get_advanced_context_engine(database_url: str, 
                                    embedding_provider: EmbeddingProvider = EmbeddingProvider.SENTENCE_TRANSFORMERS) -> AdvancedContextEngine:
    """Get or create the advanced context engine singleton."""
    global _advanced_context_engine
    
    if _advanced_context_engine is None:
        _advanced_context_engine = AdvancedContextEngine(database_url, embedding_provider)
